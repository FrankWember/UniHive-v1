...
export type CalendarEvent = {
  id: string;
  date: Date;
  startTime: string; // Format: "19:00:00"
  stopTime: string; // Format: "19:00:00" 
  title: string;
...
const Calendar = ({
  ...
  const [events, setEvents] = useState<CalendarEvent[]>(
    // Convert any old format events to new format if needed
  ...
  return (
    <div className="h-20 border-t last:border-b">
      {events
        ...
        .map((event) => {
          const eventStart = timeStringToDate(event.date, event.startTime);
          const eventEnd = timeStringToDate(event.date, event.stopTime);
          const hoursDifference = differenceInMinutes(eventEnd, eventStart) / 60;
          const startPosition = eventStart.getMinutes() / 60;
      ...
      <div className="flex sticky top-0 bg-card z-10 border-b mb-3">
        ...
        {headerDays.map((date, i) => (
          <div
            key={date.toString()}
            className={cn(
              'text-center flex-1 gap-1 pb-2 text-xs md:text-sm text-muted-foreground flex items-center justify-center',
        ...
        <div className="grid grid-cols-7 flex-1">
          {weekDates.map((hours, i) => {
            ...
            return (
              <div
                className={cn(
                  'h-full text-xs md:text-sm text-muted-foreground border-l first:border-l-0',
      ...
      <div className="grid grid-cols-7 gap-px sticky top-0 bg-background border-b">
        {weekDays.map((day, i) => (
          <div
            key={day}
            className={cn(
              'mb-2 text-center md:text-right text-sm text-muted-foreground md:pr-2',
              [0, 6].includes(i) && 'text-muted-foreground/50'
            )}
          >
            {day}
      ...
      <div className="grid overflow-hidden -mt-px flex-1 auto-rows-fr p-px grid-cols-7 gap-px">
        {monthDates.map((_date) => {
          const currentEvents = events.filter((event) =>
            isSameDay(event.date, _date)
          );

          return (
            <div
              className={cn(
                'ring-1 p-1 md:p-2 text-xs md:text-sm text-muted-foreground ring-border overflow-auto',
            ...
            >
              ...
              {currentEvents.map((event) => {
                return (
                  <div
                    key={event.id}
                    className="px-1 rounded text-xs md:text-sm flex items-center gap-1 cursor-pointer"
                  ...
                  >
                    ...
                    <time className="tabular-nums text-muted-foreground/50 text-xs hidden md:inline">
...
const TimeTable = () => {
  ...
  return (
    <div className="pr-1 md:pr-2 w-8 md:w-12">
      {Array.from(Array(25).keys()).map((hour) => {
        return (
          ...
          >
            {now.getHours() === hour && (
              <div
                className="absolute z-10 left-full translate-x-2 w-[calc(100vw-3rem)] md:w-[calc(100vw-4rem)] h-[2px] bg-red-500"
                style={{
            ...
            <p className="top-0 -translate-y-1/2 text-[10px] md:text-xs">
              {hour === 24 ? 0 : hour}:00
            </p>
          </div>
        );
