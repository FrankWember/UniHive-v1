  ...
  return (
    <div className="flex flex-col min-h-screen max-w-screen overscroll-x-none w-full">
      ...
      <div className='p-2 max-w-screen'>
        <ScrollArea className="w-full whitespace-nowrap mt-14 px-4 md:px-4 bg-muted/5 shadow-sm scroll-smooth">
          <div className='flex items-center w-max gap-10 md:gap-19 p-4 h-12'>
            {categories.map((category) => (
              <Link key={category.name} href={`/home/services?category=${category.name.toLocaleLowerCase()}`}>
                <div className='group flex flex-col items-center justify-center gap-2'>
                  <Image src={category.link} alt={category.name} width={14} height={14} className='object-cover' loading="lazy"/>
