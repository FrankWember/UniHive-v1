...
import { EmailSignInSchema, IDSignInSchema } from "@/constants/zod";
...
export const IDLogin = async (
  values: z.infer<typeof IDSignInSchema>,
  callbackUrl?: string | null
) => {
  const validatedFields = IDSignInSchema.safeParse(values);

  if (!validatedFields.success) {
    return { error: "Invalid fields!" };
  }

  const { student_id, password, code } = validatedFields.data;
